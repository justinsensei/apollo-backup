...
---
If all three of `slack`, `linear`, and `emails` outputs are empty or have no items, stop immediately. Output absolutely nothing (or just '[SILENT]') and terminate. Do not write any files, do not edit daily notes, and do not send any message.
...
6. **Daily Note Integration:**
   ...
   - For `jg_log`: `* [[inbox/YYYY-MM-DD - Slack - [Title]|Slack summary]]: <One-sentence-gist>.`
...
7. **Integrations:**
   ...
   - Run `python3 ~/.hermes/scripts/integrate_entities.py inbox/<filename> [--gist "..."]` to sync project hub states.
...
3. **ALL INGESTED NOTES MUST BE ADDED TO THE INBOX FIRST!**
   - Save the file under: `/home/justin.guest/vault/inbox/`
4. Generate the Note Content:
   ...
   - **Properties (Frontmatter):**
     ...
     original_url: "<permalink>"
     author: "<author_name>"
     associated_with: "<associated_entity_wikilink_or_title>"
     ---
     ```
   - **Note Body Layout:**
     ```markdown
     ...
     - **Associated With:** <associated_entity_wikilink_or_title>
...
5. **Daily Note Integration:**
   Append a link and one-sentence gist to today's daily note notepad under `## 🗒 Notepad`:
   - `* [[inbox/YYYY-MM-DD - Linear - [Title]|Linear Capture]]: <One-sentence-gist>.`
6. **Integrations:**
...
2. Follow the `bes-email-dispatch` skill to parse instructions and determine intents (such as filing to vault under `Inputs/Emails/` or `inbox/`, creating a Todoist task, updating contacts/projects, or creating calendar events).
3. If filing to vault:
   - Generate note under `/home/justin.guest/vault/Inputs/Emails/YYYY-MM-DD - Spaced Subject.md` (or `inbox/` if specified).
...
---
### 4. Reporting
At the end, report on all items processed from Slack, Linear, and Emails. Keep descriptions concise and formatted for Telegram delivery.
