...
The Gmail token for `personal-main` has expired or been revoked, causing the inbox poller to fail.

To restore email dispatch, re-authorize your Google Workspace connection:

```bash
python3 ~/.hermes/skills/productivity/google-workspace/scripts/setup.py --revoke
```

Then run the setup again to generate a new token.
