...
I have successfully compiled yesterday's work log (for Wednesday, May 27, 2026) and appended it directly to the daily note at `~/vault/2026-05-27 Wednesday.md`. 

The details are synthesized below across all connected sources:

***

# 📋 Work Log (2026-05-27 Wednesday)

### Today's Highlights
- **Customer.io → PostHog Sync:** Completed the instrumentation and setup for syncing Customer.io email campaign events (e.g. `cio_email_sent`, `cio_email_delivered`, `cio_email_opened`, `cio_email_clicked`, `cio_email_bounced`, `cio_email_unsubscribed`, `cio_email_spam_complaint`) into PostHog under **PROD-500**. Fully documented the event mapping schema (including top-level and metric-specific properties like `cio_event_id`, `cio_metric`, `cio_delivery_id`, and `link_url`).
- **PostHog Schema Alignments:** Opened multiple pull requests (e.g. PR #777, #778, and PRs #1-5 in `customerio-to-posthog-sync`) to align schema documentation and mapping formats to support deep marketing attribution funnel analysis.
- **Adjust & Customer.io Access for Clio:** Unblocked and configured API access for Clio (the AI research agent) to both Customer.io (**CLAW-30**) and Adjust (**CLAW-31**), receiving admin privileges on Adjust from Endre. This enables Clio to automate and enrich growth and marketing reporting.
- **Android Event Instrumentation:** Verified and marked **PROD-521** (Add `code_version_date` to all Android events) as Done.
- **Android Black Screen Video Bug Investigation:** Initiated a systematic investigation into the intermittent black screen video bug on Android (**PROD-531**), directing Clio-bot to cross-reference and append details from Zendesk Support ticket #855 (filed by Rachel Smith on Android 12 / Galaxy S10) and correlate with PostHog error events.
- **D0 → D1 Onboarding Design Discussions:** Engaged in design discussions with Anya, Tor, and Kristina regarding user focus for the "Day 0" first-lesson experience (**PROD-543**). Aligned on a strong preference for a "coach-mark" spotlight overlay (option 7) to guide and compel users to tap the first lesson tile.
- **Product Weekly Meeting:** Participated in the SignLab Product Weekly. Discussed transitioning key metrics like Day 0/Day 1 retention to 50% benchmarks and the upcoming 100% rollout of the App Redesign.
- **Personal and Family Management:**
  - Paid Sam's math tutor ($110 via Venmo to Bradley Yoder) and resolved outstanding credit card payments.
  - Coordinated Rosa's school nurse updates (Tylenol and hand-wrap for her thumb) and received teacher reminders regarding Sam's physics block review and bringing a red/white flower bouquet for the Rising Ceremony.

### Decisions Made
- **Onboarding Constraints:** Settled on restricting user actions on Day 0 to compel starting lesson 1 (preventing other clicks initially) to optimize Day 0 lesson completion.
- **App Redesign Rollout:** Tor will set the app redesign rollout to 100% across all countries as the success criteria (2x DAU growth, steady subscription revenue) has been met.
- **Pricing Strategy Experiments:** Confirmed testing a new pricing strategy (matching Lingvano's points in select markets). **Justin to set up pricing A/B test experiments** for Brazil, Norway, Germany, UK, US, and France.
- **Simon's 9th Birthday Rescheduling:** Agreed to reschedule Simon's birthday party from the busy summer/camp season to August 23rd (10 AM to 2 PM) due to widespread travel conflicts.

### Open Questions / Blockers
- **Marketing Data Source of Truth:** Discussed whether Supermetrics or Adjust should serve as the team's marketing data source of truth. Justin is conducting further research.
- **Campaign Property Sync Issue (**PROD-545**):** Identified that some desired campaign properties are not successfully syncing from Customer.io to PostHog. This is an active blocker under **PROD-545** that is planned to be resolved tomorrow.
- **LibrasLab Lock Tile Bug (**PROD-534**):** Investigating why a lock tile incorrect behavior is appearing on home for LibrasLab. Justin was unable to reproduce on iOS; suspecting it might be an Android-specific issue.
- **Physics Binder Completion:** Sam's physics block binder review might not be fully complete until Monday (per Jennifer Larson), squeezing the timeline for grading and report writing.

***

*Sources: Slack (60 msgs / 10 channels) | Linear (15 issues) | Gmail work (7 threads), personal-main (13) | Calendar (3 events / 2 accts) | Todoist (7 completed, 2 open) | daily note + chat.*
