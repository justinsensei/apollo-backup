...
If no candidates are found, skip this phase entirely and proceed to Phase 5.
...
- **Vault hygiene false positives from legacy categories.** The frontmatter `category` property is completely deprecated. gbrain now auto-classifies page types by folder paths (`meetings/`, `people/`, `projects/`, `companies/`, `concepts/`). Note that `/home/justin.guest/run_tier1_hygiene.py` still incorrectly flags notes containing the legacy `category:` frontmatter field as 'Wrong folder' issues if they live in folders other than `Notebook/` or the vault root. If there are massive numbers of "Wrong folder" issues (e.g., >1000), let Justin know these are false positives from the legacy script and offer to patch the script or strip the legacy categories.
