...
1.  **Find Unprocessed Readings:** Look for unprocessed reading files in both `Inputs/Readings/` and the Obsidian `Inbox/` (often dumped there with raw timestamp names like `Title YYYYMMDDHHmmss.md`). Check for files with frontmatter `category: "[[Readings]]"` or similar indicators. Ensure they do not already have a corresponding compiled `Source` note in `Notes/Sources/` or `Inbox/`.
...
- **Ghost Link Prevention (CRITICAL):** When renaming a Reading file or compiling a new Source note, if the original file had an ID-suffixed name (e.g., `{Title} YYYYMMDDHHmmss.md`), search the vault for any existing wikilinks pointing to that ID-suffixed name (e.g., `[[{Title} YYYYMMDDHHmmss]]`). Update those links across the vault to point to the new compiled Source note `{Title} {YYYY-MM-DD}` to prevent creating broken "Ghost Links".
