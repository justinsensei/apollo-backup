...
**Governance Rule (Human-in-the-Loop):** Work logs must **never** be written, overwritten, or updated automatically in the background (e.g., via a silent midnight cron job). They are a gray-area item requiring case-by-case review and alignment in chat before they are saved to the daily note. This review is integrated into the interactive Daily Wind-Down and Morning Briefing.
...
- **Direct Execution Blind Spots (Meetings, Bot Notifications, and Uncommitted Vault Edits):** The automated direct execution script (`direct_execution.py`) has three major blind spots that will omit critical daily decisions if relied on blindly:
  1. **Meetings:** It does not read or parse meeting files in `vault/Inputs/Meetings/` (only logging that the file was modified). You must manually search for and read meeting notes created or modified on `TARGET_DATE` to extract core business and curriculum decisions.
  2. **Slack Bot/Linear Notifications:** The script searches Slack strictly for `from:@justin` and `to:@justin`. It completely misses automated bot notifications (like Linear project status or Git release integrations) in shared channels. Manually scan the channel history (e.g., `#product-leads`) for bot posts on that date if a Linear update is mentioned.
  3. **Uncommitted/Untracked Vault Edits:** The script tracks vault changes strictly via `git log`. If the user is writing offline, or drafting new notes in their `Inbox/` that haven't been staged/committed yet, they are invisible. Proactively run a disk-based filesystem search (`search_files` or a `find` terminal command) for all markdown files modified or created on disk in the last 24 hours to ensure local drafts are fully synthesized.
