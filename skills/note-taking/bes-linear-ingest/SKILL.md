...
- **Workflow:** The agent processes incoming candidates, saves files directly to their respective `Inputs/` folder (such as `Inputs/Linear/`, `Inputs/Slack/`, `Inputs/Telegram/`, `Inputs/Emails/`), appends to today's daily note under `## 🗒 Notepad`, runs the marking-processed commands (`--mark-processed`), appends to `Utilities/log.md`, and runs `integrate_entities.py` to keep project hub states aligned.
