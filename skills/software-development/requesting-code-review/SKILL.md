...
## Codebase Composition & Inspection (pygount)

For general codebase composition audits, sizing, or language breakdown (such as counting lines of code, comment ratios, or file counts before or during code reviews), use `pygount`.
...
pygount --format=summary \
  ...
  .
```
*Note: Always use `--folders-to-skip` to exclude dependency/build directories so the tool does not hang.*
