...
Use this section when the user wants to **feel out an idea** before committing to a real build — validating feasibility, comparing approaches, or surfacing unknowns that no amount of research will answer. Spikes are disposable by design. Throw them away once they've paid their debt.
...
### 2. Build
One directory per spike under `spikes/`. Keep it standalone.
- **Bias toward something the user can interact with** (runnable CLI, minimal HTML page, small web server, or unit test with assertions).
- Hardcode configurations and dependencies. Avoid complex environment setups.

### 3. Verdict
Each spike's `README.md` must close with:
- **VALIDATED** = core question answered yes, with evidence.
- **PARTIAL** = works under certain constraints.
- **INVALIDATED** = does not work (a successful spike because it prevents building the wrong thing).
